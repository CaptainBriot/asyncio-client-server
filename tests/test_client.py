...
class TestThrottlingClient(asynctest.TestCase):
    async def setUp(self):
        self.client = client.ThrottlingClient('127.0.0.1', 8888, 10)


class TestThrottlingClientSendRequest(TestThrottlingClient):
    def test_send_request(self):
        ...
        with asynctest.mock.patch('asyncio.open_connection', return_value=(reader, writer)):
            asyncio.get_event_loop().run_until_complete(self.client.send_request())
...
class TestThrottlingClientStart(TestThrottlingClient):
    def test_start(self):
